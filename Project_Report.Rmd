...
dat1 %>% select(DataSource_name,
  ...
  mutate("ave" = sum(c(Section_Length_4,
                       ...
                       Section_Length_6,
                       Section_Length_7,
                       Section_Length_9), na.rm = TRUE) *60 /70) %>% 
  filter(ave < 2) %>% 
  ...
  labs(x = "",
       ...
       fill = "",
       title = "Distribution of respondents spent 2 seconds or less per question") +
...
straight_lining %>% 
  ...
  geom_text(size = 6, aes(label = round(Percentage, 2)), vjust = -0.4, check_overlap = TRUE, position = position_dodge(width = 0.9)) +
...
dat1 %>% 
  ...
  filter(K6_IER == 1) %>% 
  kable(caption = "Receive benefits : 0 = NO , 1 = YES") %>% 
  kable_classic_2() %>%
  kableExtra::kable_styling(latex_options = "hold_position")
...
BENTYPE_UNPAIDCARE %>% 
  ...
  theme(axis.text.x = element_text(angle=15, hjust=1)) +
  labs(fill = "") +
  geom_text(size = 6, aes(label = round(Percentage, 2)), vjust = -0.5, check_overlap = TRUE, position = position_dodge(width = 0.9))
...
dat1 %>%
  select(DataSource_name,
         starts_with("TV_TIME")) %>% 
  pivot_longer(TV_TIME_a:TV_TIME_f,
               names_to = "TV_TIME",
               values_to = "HOUR") %>% 
  ...
  pivot_wider(names_from = TV_TIME,
              values_from = HOUR) %>% 
  unnest() %>% 
  mutate("TOTAL" = round((TV_TIME_a+ TV_TIME_b+TV_TIME_c+ TV_TIME_d+ TV_TIME_e+ TV_TIME_f) / 6 * 20), 0) %>% 
  ggplot(aes(x = TOTAL)) +
...
dat1 %>%
  ...
  theme_classic() +
  labs(fill = "") +
  geom_text(size = 4, aes(label = round(Percentage, 2)), vjust = -0.5, check_overlap = TRUE, position = position_dodge(width = 0.9)) +
  theme(axis.text.x = element_text(angle=15, hjust=1)) 

dat1 %>% # See ages of those spend at 120 hours on TV_TIME
  select(DataSource_name,
         AGE,
         prob_based,
         starts_with("TV_TIME")) %>% 
