...
for (i in 1:B) {
  dd <- dat1 %>% 
    select(DataSource_name, prob_based) %>% 
    mutate(combine_flag = sample(c(1,0), n, replace = TRUE)) %>% 
    select(DataSource_name, prob_based, combine_flag) %>% 
    group_by(prob_based, DataSource_name) %>% 
    summarise(percentage = mean(combine_flag)) %>% 
    group_by(prob_based) %>% 
    summarise(percentage1 = mean(percentage))
    
  boot[i] <- dd[[2,2]] - dd[[1,2]]
}

fraudulent_boot.CI <- quantile(boot, c(0.05, 0.975))

length(boot[boot < (d[[2,2]] - d[[1,2]])]) / 10000 # p-value
# hist(boot)
```
