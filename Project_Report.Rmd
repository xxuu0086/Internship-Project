...
dat1 %>% 
  ...
  geom_point() + 
  geom_violin() + 
  geom_hline(yintercept = 70 * 2 / 60, color = "red", size = 1) +
  geom_text(aes(2,70 * 2 / 60, label = "`Cutoff` Time: 2.33 mins", vjust = 2), show.legend = FALSE) +
...
dat1 %>% select(DataSource,
  ...
  mutate(prob_based = c("Probability based", "Probability based", "Non-Probability based", "Non-Probability based", "Non-Probability based","Non-Probability based","Non-Probability based")) %>% 
